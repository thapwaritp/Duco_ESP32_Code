/*
  ...
  |  '--'  /'  ''  '|  ||  ||  |' '-' '       '  '--'\' '-' '|  ||  ||  | 
  `-------'  `----' `--'`--''--' `---'         `-----' `---' `--'`--''--' 
  Official code for ESP32 boards                            version 2.7.1
...
const char* ssid1     = "Aod-Note7";          //<-- ใส่ชือ Wifi หลัก
const char* password1 = "T0901971509";        //<-- ใส่รหัส Wifi หลัก ตรงนี
...
const char* ssid     = "Aod-Note7";            // Change this to your WiFi SSID
const char* password = "T0901971509";          // Change this to your WiFi password

String ducouser = "thanyaratja";          // Change this to your Duino-Coin username
String rigname  = "ESP32-TEST";           // Change this if you want to display a custom rig name in the Wallet

String AcctName = "thanyaratja";            //<-- ใส่ชือ User Name ตรงนี
String BrdNm    = "ESP32-TEST";             //<-- ใส่ชือ RigName ตรงนี
...
void Connect2WiFi()
{
    while (WiFi.status() != WL_CONNECTED) {
      ...
      WiFi.mode(WIFI_STA);
      Serial.print("");
      Serial.println("Start Connection WiFi");
      if (lastWiFi == 1) {
        Serial.print("Connecting to WiFi#2 :");
        delay(100);
        Serial.print(".");
...
//    Serial.print("SSID: ");
    ...
    Serial.println("      Rig name: " + String(rigname));
